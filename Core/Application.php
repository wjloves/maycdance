...
//var_dump($routeInfo);exit;
                throw new MethodNotAllowedException($routeInfo[1][0]);
    ...
    protected function registerViewBindings()
    {
        ...
        $this->singleton('view', function () {
            ...
            $twig = new \Twig_Environment ($loader, array(
                ...
                'debug' => $this->config['config.debug'],
...
//        $this->loadedConfigurations['database'] = true;
    ...
    protected function registerExcelBindings()
    {
//        $this->singleton('excel', function () {
//            return new ExcelServiceProvider($this);
//        });
        $this->singleton('excel', function () {
    ...
    /**
     * 根据配置文件中的service进行自动配置假名
     * @return void
     * Todo 需要加一个serviceBuilder
     * */
    private function registerService()
    {
        $serviceArr = $this->config['config.service'];
        $this -> aliases = array_merge($this ->aliases , $serviceArr);
    }
    ...
    protected function registerLogBindings()
    {
        $this->singleton('log', function () {
            return new Logger('h', [
                (new StreamHandler(BASEDIR . '/Cache/Logs/h.log', Logger::DEBUG))->setFormatter(new LineFormatter(null, null, true, true))
            ]);
        });
    }

    /**
     * @description 重写__get实现自动获取服务对象,判断是否有providers的方法，有则使用
     * */
    public function __get($key)
    {
        if(!isset($this[$key]) || !is_object($this[$key]) )
        {
            if(array_key_exists($key , $this->config['config.providers']))
            {
                $this->register($this->config['config.providers'][$key]);
            }else{
                $this->instance($key,$this -> make($key));
            }
