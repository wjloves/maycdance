...
class ValidateImageController extends Controller{
    ...
    /**
     *  @description 创建一个验证码图片
     * @parem $config array(
     *                   'font_size'=>文字大小（默认14）,
     *                   'img_height'=>图片高度（默认24）,
     *                   'img_width'=>图片宽度（默认68）,
     *                   'use_boder'=>使用边框（默认true）,
     *                   'font_file'=>字体文件（默认 __WEB__.'/font/bodi.ttf'）,
     *                   'filter_type'=>图片效果(0 无，1 反转颜色，2 浮雕化， 3 边缘检测, )
     *                 );
     */
    private static function createValidateImage($config = array())
    {
        #共享seesion
        Helper::shareSeesionStart();
        ...
        $font_file   = isset($config['font_file']) ? $config['font_file'] : __WEB__ . '/fonts/ggbi.ttf';
        ...
        $_SESSION['web_validate'] = $rndstring;
       ...
       // $lastc = '';
        ...
        for($i=0;$i<$rndcodelen;$i++)
        {
            ...
            $c = mt_rand(0, 15);
            $boolResult =  @imagettftext($im, $font_size, $c, $y_pos, 19, $c_fontColor, $font_file, $rndstring[$i]);
           // $lastc = $rndstring[$i];
        }
    ...
    {
        $config = array();
        $height = $this->container->request->get('height');
        if(!empty($height)){
            $config['img_height'] = $height;
        }
        $width = $this->container->request->get('width');
        if(!empty($width)){
            $config['img_width'] = $width;
        }
        $size = $this->container->request->get('size');
        if(!empty($size)){
            $config['font_size'] = $size;
        }

        self::createValidateImage($config);
    }
}
