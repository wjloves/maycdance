...
(function () {
    ...
    Editor.prototype = /**@lends baidu.editor.Editor.prototype*/{
        ...
        render : function ( container ) {
            ...
            if(container){
                container.innerHTML = '<iframe id="' + 'baidu_editor_' + this.uid + '"' + 'width="100%" height="100%" scroll="no" frameborder="0" src="javascript:(function(){document.open();document.domain=\'115.com\';var editor=parent.window.editor;editor._setup(document)})();"></iframe>';
        ...
        _setup: function ( doc ) {
            ...
            doc.write( ( ie && browser.version < 9 ? '' : '<!DOCTYPE html>') +
                ...
                '</style><script type="text/javascript">document.domain="115.com";</script></head><body' + (useBodyAsViewport ? ' class="view"' : '')  + '></body></html>' );
...
//        'Imageleft', 'ImageRight', 'ImageChar',
        ...
        'DeleteCol','SplittoRows','SplittoCols','SplittoCells','MergeCells','DeleteTable','InsertMusic'];
    k = btnCmds.length;
    while (k --) {
        cmd = btnCmds[k];
...
//插入音乐
(function(){
	QZ.pageSend = function(pageNumber, key){
    QZ.musicIndex = 1;
    var musicListTip = Y.loadTip("正在加载音乐列表...")
    $.ajax({
        url: "http://www.xiami.com/app/nineteen/search/key/" + key + "/page/" + pageNumber,
        dataType: "jsonp",
        success: function(res){

            QZ.musicClickState = true;
            musicListTip.hide();
            $(".search-music-area ul").empty();
            if (res.total != 0) {
                //var c=$(QZ.musicTemplate).tmpl(res);
                var data = res.results;
                $.each(data, function(i, n){
                    var li = $("<li><span class='song-add'><a rel=" + n.song_id + " class='add-Music' href='javascript:void(0);' title='添加'>添加</a></span><span class='song-number'>" + QZ.musicIndex + "</span><span class='song-name'></span><span class='song-author'></span></li>");
                    li.find(".song-name").html(decodeURIComponent(n.song_name).replace(key, function(){
                        return "<label class='orange'>" + key + "</label>"
                    }));
                    li.find(".song-author").html(decodeURIComponent(n.artist_name).replace(key, function(){
                        return "<label class='orange'>" + key + "</label>"
                    }));
                    $(".search-music-area ul").append(li);
                    QZ.musicIndex += 1;
                });
                if (!Y.util.isIE6) {
                    //生成分页
                    $(".music-page").data("widget_page", false);
                    $(".music-page").page({
                        current: pageNumber,
                        count: res.total,
                        size: 8,
                ...
                $(".add-Music").click(function(){
                    var text = "<embed src='http://www.xiami.com/widget/0_" + $(this).attr("rel") + "/singlePlayer.swf' type='application/x-shockwave-flash' width='257' height='33' wmode='transparent'></embed>";
                    //EDIT_CTL.Format("insertHTML", text);
					editor.execCommand("inserthtml",text);
                    QZ.musicDialog.data("widget_dialog").destroy();
                });
            }
            else {
                $(".music-page").empty();
                Y.alertTip("未找到相关结果，请更换关键词！")
                $("#js_key").focus();
            }
        }
    });
}
QZ.createMusic = function(){
    QZ.getHtml("createMusic", function(temp){
        QZ.musicDialog = $(temp).dialog({
            title: "插入网络音乐（目前支持虾米音乐）"
        });
        //JSONP
        QZ.musicClickState = true;
        ...
        Y.keyup("#js_key", "return", function(){
            getResult();
        })
        var getResult = function(keyEl){
            var key = $.trim($("#js_key").val());
            if (key.length < 1) {
                Y.alertTip("请输入查询的关键字");
                $("#js_key").focus();
                return false;
            }
            if (QZ.musicClickState) {
                QZ.pageSend(1, key);
            }
            QZ.musicClickState = false;
        }
    }, "/static/resources/all/");
}

	baidu.editor.plugins['insertmusic']=function(){
		var me=this;
		//me.addEventListener("click",function(type,evt){
			
		//})
		
		baidu.editor.commands["insertmusic"]={
			execCommand: function (cmdName, tableobj) {
				QZ.createMusic();
			}
		}
...
//插入附件
(function(){
	baidu.editor.plugins['insertattach']=function(){
		var me=this;
	}
})();