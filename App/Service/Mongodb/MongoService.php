 ...
 * @author
 * $Id: cls_mongodb.php 1 2013-07-18 09:24:31Z  $
 */
...
class MongoService
{
    ...
    {
        static $dbs = array();
        $GLOBALS['config']['mongo_host'] = 'mongodb://10.72.3.29:27017';
        $GLOBALS['config']['mongo_db'] = 'u_leajoy';
        self::$dbname = empty(self::$dbname)?(empty($dbname)?$GLOBALS['config']['mongo_db']:$dbname):self::$dbname;
        if( empty($dbs[self::$dbname]) )
        {
            if(empty($host)) $host = $GLOBALS['config']['mongo_host'].'/'.self::$dbname;
            ...
            if( self::$con === null )
            {
                ...
                self::$con = new MongoClient( $host, array('connectTimeoutMS'=>$wait_time, 'connect'=>true) );
                var_dump(self::$con);die;
            }

            $dbs[self::$dbname] = self::$con->selectDB( self::$dbname );
            //$dbs[self::$dbname]->wtimeout = self::$wait_time;
        }

        return $dbs[self::$dbname];
    }
